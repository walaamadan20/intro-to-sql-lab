...
SELECT * FROM countries WHERE region = 'Southren Europe' 
ORDER BY population ACS
LIMIT 1
...
INNER JOIN countries ON countrylanguages.countrycode = countries.code
WHERE code = 'VAT' AND isofficial = 'true'
...
INNER JOIN countries ON countrylanguages.countrycode = countries.code
WHERE language = 'Italian' AND isofficial = 'true' AND region = 'Southern Europe'
...
WHERE co.continent = 'South America'
  AND ci.name ILIKE '%serra%'     -- similar to 'Serravalle'
  AND ci.name NOT ILIKE '%valle'; -- doesn't end with 'valle'
...
FROM cities
WHERE population BETWEEN 91000 AND 91100;


-- and she most likely have gone to Snata Monica